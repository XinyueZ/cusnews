...
public class MainActivity extends CusNewsActivity implements SearchView.OnQueryTextListener, TabLabelManagerUIHelper {
	...
	protected void onCreate( Bundle savedInstanceState ) {
		...
		getSupportFragmentManager().beginTransaction()
								   .replace(
										   R.id.bookmark_list_container_fl,
										   BookmarksFragment.newInstance( this )
								   )
								   .commit();
			...
			public void onReceive( Context context, Intent intent ) {
				if( !TextUtils.isEmpty( Prefs.getInstance()
					...
					Snackbar.make(
							...
							R.string.lbl_register_push_failed,
									...
									new OnClickListener() {
										...
										public void onClick( View v ) {
											mPb = ProgressDialog.show(
													MainActivity.this,
													null,
													getString( R.string.lbl_registering )
											);
		...
		switch( vt ) {
			case GRID:
				mBinding.entriesRv.setLayoutManager( mLayoutManager = new GridLayoutManager(
						this,
						GRID_SPAN
				) );
		...
		mBinding.contentSrl.setProgressViewEndTarget(
				true,
				mActionBarHeight * 2
		);
		mBinding.contentSrl.setProgressViewOffset(
				false,
				0,
				mActionBarHeight * 2
		);
		...
		mDrawerLayout.setDrawerListener( new SimpleDrawerListener() {
			...
			public void onDrawerOpened( View drawerView ) {
				...
				ViewPropertyAnimator.animate( mThumbIv )
									.cancel();
				ViewPropertyAnimator.animate( mThumbIv )
									.alpha( 1f )
									.scaleX( 1f )
									...
									.setDuration( 500 )
									.start();
				ViewPropertyAnimator.animate( mExitV )
									.cancel();
				ViewPropertyAnimator.animate( mExitV )
									.alpha( 1f )
									.scaleX( 1f )
			...
			public void onClick( View v ) {
				if( Utils.validateKeyword( mBinding.newTabLabelTv ) ) {
					if( !TextUtils.isEmpty( mBinding.newTabLabelTv.getText() ) ) {
						TabLabel newTabLabel = new TabLabel(
								mBinding.newTabLabelTv.getText()
													  .toString()
													  .trim(),
								Prefs.getInstance()
									 .getGoogleId()
						);
						TabLabelManager.getInstance()
									   .addNewRemoteTab(
											   newTabLabel,
											   MainActivity.this,
											   mBinding.coordinatorLayout
		...
		Api.getTopTrends(
				...
				Prefs.getInstance()
					 .getLanguage(),
				App.Instance.getApiKey(),
				new Callback<Trends>() {
					@Override
					public void success( Trends trends, Response response ) {
						List<String> list          = trends.getList();
						MenuItem     trendsMenu    = mBinding.navView.getMenu()
																	 .findItem( R.id.action_trends );
						SubMenu      trendsMenuSub = trendsMenu.getSubMenu();
						for( String trend : list ) {
							trendsMenuSub.add( trend )
										 .setIcon( R.drawable.ic_social_whatshot )
										 .setVisible( true )
															 ...
															 new DialogFragment() {
																 ...
																 public Dialog onCreateDialog( Bundle savedInstanceState ) {
																	 ...
																	 builder.setTitle( R.string.application_name )
																			.setMessage( R.string.lbl_tabs_tip )
																			.setPositiveButton(
																					R.string.btn_ok,
																					null
																			);
	...
	public void onEvent( ChangeViewTypeEvent e ) {
		...
		EntriesAdapter newAdp = new EntriesAdapter(
				e.getViewType(),
				mBinding.getEntriesAdapter()
						.getData()
		);
	...
	public void onEvent( BookmarksInitEvent e ) {
		if( mBinding.getEntriesAdapter() != null ) {
			mBinding.getEntriesAdapter()
	...
	private void loadAllData() {
		...
		if( !TextUtils.isEmpty( prefs.getGoogleThumbUrl() ) ) {
			picasso.load( com.chopping.utils.Utils.uriStr2URI( prefs.getGoogleThumbUrl() )
												  .toASCIIString() )
				   .into( mThumbIv );
		}
	...
	public void addDefaultTab() {
		mBinding.tabs.addTab( mBinding.tabs.newTab()
	...
	public Tab addTab( TabLabel tabLabel ) {
		View      tabV  = getLayoutInflater().inflate(
				R.layout.tab,
				null,
				false
		);
	...
	public boolean onOptionsItemSelected( MenuItem item ) {
		...
		switch( item.getItemId() ) {
			...
			case R.id.action_to_top:
				if( mBinding.getEntriesAdapter() != null && mBinding.getEntriesAdapter()
																	.getItemCount() > 0 ) {
					if( mLayoutManager instanceof LinearLayoutManager ) {
						( (LinearLayoutManager) mLayoutManager ).scrollToPositionWithOffset(
								0,
								0
						);
					}
	...
	protected void handleIntent( Intent intent ) {
		...
		if( !TextUtils.isEmpty( mKeyword ) && Utils.validateStr(
				App.Instance,
				mKeyword
		) ) {
			...
			public boolean onNavigationItemSelected( MenuItem menuItem ) {
				...
				switch( menuItem.getItemId() ) {
					case R.id.action_faroo_home:
						WebViewActivity.showInstance( MainActivity.this,
													  getString( R.string.action_visit_faroo ),
													  Prefs.getInstance()
														   .getFarooBlog()
						);
						break;
							...
							showDialogFragment(
									new DialogFragment() {
										...
										public Dialog onCreateDialog( Bundle savedInstanceState ) {
											// Use the Builder class for convenient dialog construction
											AlertDialog.Builder builder = new AlertDialog.Builder( getActivity() );
											builder.setTitle( R.string.action_push )
												   .setSingleChoiceItems( R.array.setting_pushs,
																		  -1,
																		  new DialogInterface.OnClickListener() {
																			  @Override
																			  public void onClick( DialogInterface dialog, int which
	...
	private void askPush() {
		...
		builder.setTitle( R.string.application_name )
			   .setMessage( R.string.lbl_ask_push )
			   .setNeutralButton(
					   R.string.btn_diy,
					   new DialogInterface.OnClickListener() {
						   @Override
						   public void onClick( DialogInterface dialog, int which
							   ...
							   dismissPb();
							   mPb = ProgressDialog.show( MainActivity.this,
														  null,
														  getString( R.string.lbl_registering )
							   );
							   mPb.setCancelable( true );
